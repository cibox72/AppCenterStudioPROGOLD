...
// ============================================
// 36-37. LISTA REGALI (AGGIORNATO CON DONAZIONI SINGOLE)
// ============================================
...
if (path === "/api/studio/lista-regali" && request.method === "POST") {
    ...
    const d = await request.json();
    await env.DB.prepare(`INSERT INTO lista_regali (id, studio_id, cliente_id, cliente_nome, tipo_evento, importo_servizio, metodo_pagamento, dati_pagamento, messaggio_cortesia, username, password, link_pubblico, raccolto_attuale, stato, data_creazione) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(
        d.id, d.studio_id, d.cliente_id, d.cliente_nome, d.tipo_evento, d.importo_servizio, d.metodo_pagamento, d.dati_pagamento, d.messaggio_cortesia, d.username, d.password, d.link_pubblico, d.raccolto_attuale || 0, d.stato || 'in_corso', d.data_creazione || new Date().toISOString()
    ).run();
...
if (path.startsWith("/api/public/lista-regali/") && request.method === "GET" && !path.includes("findByCredentials") && !path.includes("donazioni") && !path.includes("messaggi")) {
...
if (path === "/api/public/lista-regali/donazione" && request.method === "POST") {
    ...
    // Aggiorna automaticamente il totale nella tabella lista_regali
...
// 8. ELIMINA DONAZIONE (Dashboard Cliente - in caso di errore)
